...
- **1.5 (2026-09-11)** — new per-game mode `720p`: the plugin switches the
  real output to 720p60 when a listed title starts and back to 1080p30 when it
  exits, driven by the kernel's process events (the switch is issued from the
  event callback and, if that context refuses it, from the next display call
  of any process). For games that cannot start under the 1080p30 head at all
  (Tales of Hearts R crashes after the trophy screen without ever touching the
  display, so no query spoof can reach it). While such a title runs the
  watchdog treats 720p as the wanted state.
...
| `720p` | Run this title at real 720p60: the output switches when the game starts and returns to 1080p30 when it exits. Use for games that crash before drawing anything under 1080p30 (Tales of Hearts R). |
